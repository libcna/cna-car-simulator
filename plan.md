...
- [ ] `ENV-001` Baked lighting: world-space lightmap (sun visibility + AO) for terrain/roads; per-object lighting for buildings/props.
- [ ] `ENV-002` Planar stencil shadows for vehicles.
- [ ] `ENV-003` Czech town content: square, houses (families), shops, church, panel blocks, bus stop, street furniture.
- [ ] `ENV-004` Countryside: fields, meadows, tree avenues, delineators, ditches.
- [ ] `ENV-005` Forest: spruce/pine/beech generators, forest road, timber stacks, barrier gate.
- [ ] `ENV-006` Sign set (P1, P2, P4, P6, B20a/b, IZ4a/b, IS3, IP6, IJ4c, A-series) procedural faces + posts.
- [ ] `ENV-007` Road markings (V1a, V2a/b, V4, V5, V7) as decals with depth bias.
