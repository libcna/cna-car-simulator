...
The sample map is generated by a two-stage pipeline under `tools/maps/`, driven by
`build_map.py`: stage 1 (`generate_lipova.py`) writes the town and its countryside from scratch,
stage 2 (`add_settlements.py`) grows that into the wider region and is additive and idempotent.
The committed JSON is the pipeline's output and `map_regeneration_check` fails if the two drift
apart. See [map-generation.md](map-generation.md).
...
- `tools/maps/build_map.py` generates the sample map from its two source stages; the generated
  JSON is committed and is what the simulator loads. `--check` (CTest `map_regeneration_check`)
  fails if the committed map is no longer what the pipeline produces.
