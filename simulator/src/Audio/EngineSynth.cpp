...
namespace CarSim::Audio
{
    ...
    void EngineSynth::Render(float* out, const int frames, const EngineSoundInput& target)
    {
        ...
        const float combustionTarget = target.state == EngineSoundState::Running ? 1.0f :
                                       target.state == EngineSoundState::Starting ? 0.15f : 0.0f;
        const float combustionRate = dt / 0.010f;
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            // Keep a trace of compression/firing while cranking, then reach full running
            // level quickly. The catch clip and running timbre retain their original strength.
            float sample = combustionGain_ *
                           (harmonics * 0.30f * (0.45f + 0.55f * load) + pulses * 0.22f + whine + intake);
